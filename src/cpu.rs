...
impl CPU {
    ...
    pub fn shr(&mut self, reg1: usize, reg2: usize) {
        self.gen_purpose_registers[15] = self.gen_purpose_registers[reg2] % 2;
    ...
    pub fn shl(&mut self, reg1: usize, reg2: usize) {
    ...
    pub fn rnd(&mut self, reg1: usize, byte: u8) {
    ...
    pub fn ld_delay_toreg(&mut self, reg1: usize){
    ...
    pub fn ld_delay(&mut self, reg1: usize){
        self.delay_timer = self.gen_purpose_registers[reg1];
    }

    // Moves sound timer value from reg1
    pub fn ld_sound(&mut self, reg1: usize){
        self.sound_timer = self.gen_purpose_registers[reg1];
    }

    // Adds the value at Vx to I
    pub fn add_i(&mut self, reg1: usize) {
    ...
    pub fn ld_bcd(&mut self, reg1: usize) {
        let mut val: u8 = self.gen_purpose_registers[reg1];

        self.memory[self.register_i as usize] = val / 100;
        val = val % 100;
        self.memory[(self.register_i + 1) as usize] = val / 10;
        val = val % 10;
        self.memory[(self.register_i + 2) as usize] = val;
    }
